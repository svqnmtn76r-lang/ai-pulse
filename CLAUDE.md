...
### 2.5.0 現在の状態（2026-06-13 更新）★ 最新

**独自ドメイン `aitickerhq.com` へ移行済み + サイト名を「AI Ticker HQ」へリブランド**。

- 本番URL: `https://aitickerhq.com`
- サイト名: **AI Ticker HQ**（旧 AI-Pulse）
- タグライン: **AI News & Tools Intelligence**
- 旧サブドメイン `ai-pulse-b35.pages.dev` は 301 リダイレクトで新ドメインへ転送（`blog/public/_redirects`）
- GSC: 新旧両プロパティの verification meta を `BaseHead.astro` に併記（移行完了後に旧トークン削除予定）

> 注: リポジトリ名・ローカルディレクトリ名は `ai-pulse` のまま（変更コスト回避）。サイト表示名のみリブランド。

以下 2.5.1〜2.5.3 は移行前の意思決定記録（履歴として保存）。

### 2.5.1 当初の選択（2026-05-21、移行により終了）
...
### 2.5.2 独自ドメイン取得トリガー（達成済み → 2026-06-13 に移行実施）

当初は以下のいずれか達成時に独自ドメインへ移行する計画だった。実際には予定を前倒しし、
最終確定名称を `aitickerhq.com` として 2026-06-13 に移行を実施した：
...
**最終確定（2026-06-13）**: `aitickerhq.com` を採用・移行。「AI Ticker HQ」ブランドと表記一致。

当初（2026-05-21）の候補リスト（履歴）：

| ドメイン | 価格/年 | コメント |
|---|---|---|
| aipulse-stack.com | $10.46 | AutoAffilと表記揃う、当時の第一候補（不採用）|
| aipulsestack.com | $10.46 | ハイフン無し版（不採用）|
| aipulse-stack.app | $14.20 | .app は最終プロダクト感（不採用）|

他者所有のため回避（2026-05-21 時点）：
- ai-pulse.dev
- aipulse.news
- aipulse.ai
