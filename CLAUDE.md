...
実際に生成された成果物（記事/データ/レポート）を **実物検査** で採点。
「動いている」と「価値ある出力を出している」を別物として測る。

| 評価項目 | 配点 | Day 2 での失敗例 |
|---|---|---|
| 主要機能の有効性<br>（例：アフィリ商品マッチ率、スコア分散、カテゴリ多様性）| 20 | products: [] が100% → 0点が妥当だったが見過ごし |
| 出力多様性<br>（テンプレ・データソース・長さの分散）| 15 | template_type breaking 100%、word_count 298-418狭域 |
| 法的・倫理リスク<br>（FTC開示、コピペ転載、ハルシネーション）| 10 | products=[] でも開示文付与 = FTC違反疑い |
| 冪等性・再現性<br>（同じ入力で重複出力しないか）| 10 | 重複ファイル 16/45 発生 |
| 公開可能レベル<br>（このまま公開して恥ずかしくないか）| 5 | ChatGPT臭・独自視点ゼロで公開不可レベル |
...
### 0.4 Day 2 で判明した実装採点の修正履歴

Day 2 自己採点 99/100 が実態 63/100 だった事例（2026-05-23判明）：
...
| 2026-05-25 | Day 2.6 修正実行（4つのバグ修正）: **B1** products_mentioned フィールド衝突解消、**B2** score≥40 閾値ゲート実装、**B3** matcher min_score 30→15 + HN キーワード厳格化、**B4** API 呼び出し最適化（72→5回）| - |
| 2026-05-25 | Day 2.6 完了（自己採点 68/100「不合格」）: 軸A 40/40 完全達成、軸B 28/60 不合格。理由：商品マッチ率 0%（B-1: 0/20）、カテゴリ分散 2種<3（B-2: 0/15）、スコア分散 2値<3（B-3: 0/10）。テンプレ分散 O（B-4: 8/8）、重複なし（B-5: 10/10）、FTC 違反なし（B-6: 5/5）、公開可能なし（B-7: 0/7）。根本原因：RSS データソース 3 つ全部失敗（Day 1 既知課題）→ GitHub リリース過剰、HN 1 件のみ。Day 3 で二次媒体追加で改善予定。 | 68/100（軸B不合格） |
